...
DSN_DB = DSN + '{name}'
CREATE_DB = 'CREATE DATEBASE {name}'
    ...
    async def connect(self) -> Pool:
        if self.owner:
            await self.server_command(
                CREATE_DB.format(**self.parms))

        self.pool = await asyncpg.create_pool(
            DSN_DB.format(**self.parms))
        return self.pool

    async def disconnect(self):
        """Destroy the database"""
        if self.pool:
            release = [self.pool.release(conn)
                       for conn in self.listeners]
            await asyncio.gather(*release)
            await self.pool.close()
        if self.owner:
            await self.server_command(
                DROP_DB.format(**self.parms))
